...
Las decisiones más útiles surgieron al reducir el alcance. Flask encajó mejor que un framework completo porque la API tiene pocas rutas y no necesita un modelo relacional. La persistencia evolucionó desde una colección JSON única hacia Cosmos DB porque el modelo documental seguía siendo sencillo, pero convenía separar cada solicitud en un documento propio. El pipeline de GitHub Actions previsto al principio se descartó a favor de scripts PowerShell y Azure CLI que podían ejecutarse, revisar y demostrar desde el entorno disponible.
