...
\begin{abstract}
This Final Degree Project addresses the fragmented handling of internal IT requests through a working prototype deployed on Microsoft Azure. The solution combines a Flask API and web portal hosted on App Service, document persistence in Blob Storage, secret management with Key Vault, managed identity, and a Logic App that accepts requests from external systems. A rule-based classifier assigns a request type, category, priority, and operational recommendation. The workflow continues with approval, assignment, escalation, SLA tracking, closure, and user feedback. The project was developed over five iterations tracked in Zube and includes OpenAPI documentation, reproducible deployment and verification scripts, twenty-six automated tests, Application Insights monitoring, and SonarCloud code analysis. The report discusses the adopted decisions, the rejected alternatives, and the limitations that distinguish the prototype from a production ITSM platform.
\end{abstract}
